...
export default function UserDashboard() {
  ...
  const { data: bookings, isLoading, error, reload } = useFetch(() => getMyBookings(user?.id), [user?.id]);

  const wishlistTours = TOURS.filter((t) => user?.wishlist?.includes(t.id));
          ...
          {activeTab === 'bookings' && (
            <div className="space-y-4">
              ...
              {bookings?.map((booking) => (
                <div key={booking.id} className="card flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="font-semibold text-lagoon-900">{booking.tourTitle}</p>
                    <p className="mt-1 text-xs text-lagoon-500">
                      Booking ID: {booking.id} · {booking.travelers} traveler{booking.travelers > 1 ? 's' : ''} · {formatDate(booking.startDate)}
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="font-display text-lg font-semibold text-lagoon-800">{formatCurrency(booking.totalPrice)}</span>
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${STATUS_BADGE_STYLES[booking.status]}`}>
            ...
            <div className="space-y-4">
              {wishlistTours.length === 0 ? (
                ...
                wishlistTours.map((tour) => (
                  <Link key={tour.id} to={`/tours/${tour.id}`} className="card flex items-center gap-4 p-4 transition hover:shadow-soft">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-3 text-xs text-lagoon-500">
                        ...
                        <span className="flex items-center gap-1"><Clock3 size={12} /> {tour.duration}d</span>
                      </div>
                    </div>
